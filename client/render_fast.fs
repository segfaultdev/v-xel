  ...
  for (int i = 0; i < vx_iterations; i++) {
    ...
    if (tile != vx_tile_air) {
      ...
      if (tile == vx_tile_leaves) {
        if (hit_side == 0) {
          if (in_leaves((hit.y - floor(hit.y)) * 16.0, (hit.z - floor(hit.z)) * 16.0)) color = vec3(0.0, 0.0, 0.0);
        } else if (hit_side == 1) {
          if (in_leaves((hit.z - floor(hit.z)) * 16.0, (hit.x - floor(hit.x)) * 16.0)) color = vec3(0.0, 0.0, 0.0);
        } else if (hit_side == 2) {
          if (in_leaves((hit.x - floor(hit.x)) * 16.0, (hit.y - floor(hit.y)) * 16.0)) color = vec3(0.0, 0.0, 0.0);
        }
