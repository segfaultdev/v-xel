...
Color plot_pixel(float cam_x, float cam_y, float cam_z, int scr_x, int scr_y, float dir_x, float dir_y, float dir_z, int limit) {
  ...
  for (i = 0; i < iter; i++) {
    ...
    if (tile) {
      ...
      float daylight = 1.0f; // fast_abs(2.0f * daytime - 1.0f);
      
      /*
      float curr_hue = get_hue(c.r, c.g, c.b);
      float recv_hue = 0.1f;
      
      float hue_diff = fast_abs(curr_hue - recv_hue);
      if (1.0f - hue_diff < hue_diff) hue_diff = 1.0f - hue_diff;
      */
      ...
      if (do_shadows) {
        ...
        if (!((tile >= 8 && tile <= 10) || (!(hit_side == 1 && step_y > 0) && top.a == AIR_COLOR.a))) {
          light = 1.0f - ((top.a * 0.4f) / 255.0f);
        }
      }
      
      float temp = 1.0f - fast_cos(PI * (daytime + 0.25f)) * fast_cos(PI * (daytime + 0.25f));
      float lumi = (c.r + c.g + c.b) / 3.0f;
      
      float sat_r = 0.35f * temp + 0.65f;
      float sat_g = 0.40f * temp + 0.60f;
      float sat_b = 0.20f * temp + 0.80f;
      
      c.r = c.r * sat_r + lumi * (1.0f - sat_r);
      c.g = c.g * sat_g + lumi * (1.0f - sat_g);
      c.b = c.b * sat_b + lumi * (1.0f - sat_b);
      
      float max_light = temp * 0.40f + 0.60f;
